<template>
  <div class="card">
    <div class="card-header">
      ...
      >{{ creating.active ? 'Cancel' : '+ Add New' }}</a>
    </div>
    <div class="card-block">
        ...
        <div class="card-block">
          <form action="#" @submit.prevent="store()">
            <div
              ...
              :class="{ 'has-danger': creating.errors[column]}"
            >
              <label
      ...
      <div v-if="response.allow.filter">
    ...
    <div class="card-block card-block-light" style="margin: .625rem; margin-top: 0;padding: 0;">
      <ul class="list-group">
        <li class="list-group-item row hidden-lg-down row">
          <div class="row">
            <div class="col-lg" v-for="column in response.displayable" v-bind:key="column.index">
              <span
                class="sortable"
                @click="sortBy(column)"
              >{{ response.custom_columns[column] || column }}</span>
              <div
                class="arrow"
        ...
        <li class="list-group-item" v-for="record in filteredRecords" v-bind:key="record.index">
          <div class="row">
            <div
              class="col-lg ellipsis"
              v-for="(columnValue, column) in record"
              v-bind:key="column"
              v-bind:title="columnValue"
            >
              <label class="hidden-xl-up">{{ column }}:</label>
              <template v-if="editing.id === record.id && isUpdatable(column)">
                <div class="form-group" :class="{ 'has-danger': editing.errors[column]}">
                  <template
                    v-if="response.column_types[column] && response.column_types[column].type === 'select'"
                  >
                    <select
                      class="form-control"
                      :value="columnValue"
                    ...
                    >
                      <option
                        v-for="option in response.column_types[column].options"
                        v-bind:value="option.index"
                      >{{ option }}</option>
                    </select>
                  </template>
                  <template v-else>
              ...
              <template v-else>
                <template v-if="response.column_links[column] && columnValue">
                  <a v-bind:href="account_link(column, columnValue)">{{ formatted(columnValue, column) }}</a>
                </template>
                <template v-else>{{ formatted(columnValue, column) }}</template>
              </template>
            </div>
            <div class="col-lg" v-if="response.allow.update && response.buttons.length == 0">
              <a
                href="#"
            ...
            <div class="col-lg" v-if="response.buttons.length > 0">
              <div class="btn-group" v-for="btn in response.buttons" v-bind:key="btn.index">
                <a
                  v-bind:href="btn.route + record.id + btn.method"
                  class="btn btn-sm btn-primary"
                >{{ btn.label }}</a>
              </div>
            </div>
...
export default {
    ...
    filteredRecords() {
      ...
      if (this.sort.key) {
        data = _.orderBy(
          ...
          (i) => {
            if (
              this.sort.key.toLowerCase().indexOf("date") > -1 ||
              this.sort.key.toLowerCase().indexOf("stamp") > -1 ||
              this.sort.key === "created_at"
            ) {
              var value = moment(i[this.sort.key]);
              var num =
                isNaN(parseFloat(value.format("YYYYMMDD"))) === true
                  ? 0
                  : parseFloat(value.format("YYYYMMDD"));
              return num;
            } else {
  ...
  methods: {
    ...
    formatted(value, column=null) {
      if (column
        && this.response.column_types[column]
        && this.response.column_types[column]['format'])
      {
        if (this.response.column_types[column]['format'] == 'date') {
    ...
    account_link(column, value) {
      return this.response.column_links[column].replace(
        "-+" + column + "-+",
        value
      );
    },

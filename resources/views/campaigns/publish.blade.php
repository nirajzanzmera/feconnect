                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="schedule pull right">
                            ...
                            <strong>Send Time:</strong> {{ $campaign->send_time }} {{ Auth::user()->currentTeam->send_timezone }}<br />
    ...
    <div class="col-md-5">
        <div class="card card-info">
            <div class="card-block">
                ...
                @endif
                <a href="{{ route('campaigns.publish', $campaign) }}" class="btn btn-success {{ ($errors->count() > 0) ? 'disabled' : '' }}">
                    <i class="fa fa-calendar"></i> 
                    Schedule Send
                </a>
                @if  (!empty($errors->first('schedule')) && !$errors->has('sender_id')  && !$errors->has('subject') && $errors->count() <= 1 && Auth::user()->currentTeam == 'active')
                <a href="{{ route('campaigns.publish', ['campaign' => $campaign , 'immediate'=> true]) }}" class="btn btn-success">
...
@include('layouts.partials._preview', ['model'=>$campaign, 'route_name'=>'campaigns.iframe'])
@if($campaign->status != 'PUBLISHED' && $campaign->status != 'locked')
<a href="{{ route('campaigns.edit', $campaign) }}" class="btn btn-primary"><i class="fa fa-edit"></i> Edit</a>
@endif
<a href="{{ route('campaigns.publish', $campaign) }}" class="btn btn-success {{ ($errors->count() > 0) ? 'disabled' : '' }}"><i class="fa fa-calendar"></i> Schedule Send</a>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.update').on('click', function(e){
            ...
            $.ajax({
                url: '{{ route('campaigns.update_schedule', $campaign->id) }}',
                data: {
                    send_date: date,
                    send_time: time,
                },
                ...
                success: function(result) {
                    window.location = '{{ route('campaigns.publish_preview', $campaign) }}';
                }
            });  
        });
        ...
        var tmpl_html = render_html(`{!! preg_replace('/\s\s+/', ' ',  $campaign->html) !!}`);

...
@if(!isset($headless) || (isset($headless) && $headless != true))
    ...
    <ul class="nav nav-pills flex-sm-row" style="padding-bottom: 15px;">
        <li class="nav-item">
            <a class="nav-link{{ Route::currentRouteName() == 'websites.dashboard' ? ' active' : ''}}"
                href="{{ route('websites.dashboard') }}">
            ...
            <div class="card">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                ...
                                <a class="dropdown-item{{ !empty($website) && $site->id == $website->id ? ' active' : ''}}"
                                    href="{{ route('websites.analytics', [ 'website' => $site->id /*, 'sd' => $start->format('Y-m-d'), 'ed' => $end->format('Y-m-d') */ ]) }}">
                                    {{ $site->name }}
                                </a>
                                @endforeach
                            </div>
                ...
                @if (!empty($website))
                    <div class="card-block">
                        ...
                        @if(!empty($page))
                        <a target="_blank" href="{{ !empty($page) and !empty($page->preview_link) ? $page->preview_link : $website_preview }}"
                            @if($dark)style="color:white" @endif>
                            {{ !empty($page) and !empty($page->preview_link) ? $page->preview_link : $website_preview }}
                        </a>
                            @if(!empty($site_link))
                        ...
                        @elseif( !empty($post) && $post->status == 'live' && ( !empty($post->start_time) && \Carbon\Carbon::parse($post->start_time->date)->format('Y-m-d') <= date("Y-m-d") ))
                            <a target="_blank" href="{{ (!empty($post) && !empty($post->preview_link)) ? $post->preview_link : $website_preview }}"
                                @if($dark)style="color:white" @endif>
                                {{ (!empty($post) && !empty($post->preview_link)) ? $post->preview_link : $website_preview }}
                            </a>
                        ...
                        @else
                            @if(!empty($website->preview))
                                <a target="_blank" href="{{ $website_preview }}" @if($dark)style="color:white" @endif>
                                    {{ $website_preview }}
                                </a>
                            @endif
                        @endif
                            ...
                            @if(!empty($page))
                            <a target="_blank" href="{{ !empty($page) ? $page->site_link : $website_preview }}"
                                @if($dark)style="color:white" @endif>
                                {{ !empty($page) ? $page->site_link : $website_preview }}
                            </a>
                            @else

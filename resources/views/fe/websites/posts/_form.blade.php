...
<div id="app" v-on:change="stuffChanged()" data-href="{{isset($post->id) ? route('api.website.post', ['id'=>$website->id, 'post' => $post->id ]) : ''}}">
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="row form-group">
                ...
                <div class="col-lg-4">
                    <input id="datepicker" name="publish_date" class=" form-control" type="text"
                        value="{{ old('publish_date', !empty($post->start_time->date) ? date('m/d/Y',strtotime($post->start_time->date)) : NULL ) }}"
                        autocomplete="nope">
                </div>
                <div class="col-lg-2">
                ...
                <div class="col-lg-4">
                    <div class="input-group">
                        <input name="publish_time" id="timepicker" type="text" class="form-control"
                            value="{{ old('publish_time', !empty($post->start_time->date) ? date('H:i A',strtotime($post->start_time->date)) : NULL ) }}"
                            autocomplete="off">
                        <span class="input-group-addon">{{ 'HDT' }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <fieldset class="form-group{{ $errors->has('category') ? ' has-error' : '' }}">
                        ...
                        <select name="category" id="category" class="form-control" v-if="!category">
                            ...
                            @if(!empty($categories))
                                ...
                                @if( old('category', 
                                    (!empty($post) && !empty($post->categories)) ? $post->categories[0]->name : '') == $cat->name )
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        //     console.log(action_btn.data('action'));
        //     if (action_btn.data('action') == "edit") { 
        //          let message = "Any unsaved changes would be discarded. Are you sure you want to leave this page?";
        //          return message;
        //     }
        // }, false);
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.post) {
                        if (action == 'edit') {
                            $('#successMessage').show();
                            vue_app.formChange = false;
                        } else {
                            var post_edit_url = '{{ route('websites.posts.index', ['website'=>$website->id]) }}';
                            window.location = post_edit_url + '/' + result.post.id + '/edit';
                        }
                    }
                    setTimeout(()=> {
                        $('#successMessage').hide();
    ...
    */
    ...
    function render_preview(){
        $.post('{{ route('website.preview', $website->id) }}', {data: JSON.stringify(vue_app.site_data), type: 'post'}, function(data){

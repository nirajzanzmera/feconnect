...
<div class="row-fluid">
    <div class="col-md-12">
        <div class="card">
            ...
            @include('layouts.partials._preview', ['model'=> $template->id , 'route_name'=>'newsletter.iframe'])
...
$(document).ready(function(){
    var tmpl_html = render_html(atob("{{ base64_encode($html) }}"));
    ...
    $('#feed_select').on('change', function(){
        ...
        if(type == 'website') {
            {{--var newsletter_url = "{{ route('newsletters.create.website.preview', ['template_id'=>$tid,'website'=>1]) }}/"--}}
            var newsletter_url = "{{ url('emails/newsletters/create/preview/'.$tid.'/website/') }}/";
        } else {
{{--            var newsletter_url = "{{ route('newsletters.create.feed.preview', ['template_id'=>$tid,'feed'=>1]) }}/"--}}
            var newsletter_url = "{{ url('emails/newsletters/create/preview/'.$tid.'/feed/') }}/";
        }

...
<div id="app">
    {{-- <form action="{{ route('websites.pages.update', ['website'=>$website, 'page'=>$page, 'type'=>$type]) }}"
                        ...
                        <div v-if="site_data.advanced == true">
                            <div class="section">
                                ...
                                <div v-if="site_data.showGallery">
                                    ...
                                    <ul class="list-group">
                                        <li v-for="(item, index) in formData.settings.gallery" class="list-group-item">
                                            <div class="row">
                                                <div v-if="index == currentItem" class="col-md-12">
                                                    <standalone v-on:standalone="itemUpdate" v-on:close="itemUpdate"
                                                        :name="index" search_url="{{ route('content.images.search') }}"
...
<script>
    ...
    var WebsiteDataUrl = '{!! route('websites.data', ['website'=>$website, 'page'=>!empty($page) ? $page : '', 'template' => !empty($template) ? $template : '' ]) !!}';
    @include('websites._templates')
    @include('websites._tiny5_init')
    var crud_type = '{{ $crud_type }}';
    var menu_id = {{ $menu_id ?? 0}};
    @if($crud_type == 'edit') 
    var title_check_url = '{{ route('websites.pages.title_check', $website) }}?page={{ $page->id }}';
    var page_url = '{{ route('websites.pages.update', ['website'=>$website, 'page'=>$page, 'type'=>$type]) }}';
    var page_url2 = '{{ route('websites.pages.edit', ['website'=>$website, 'page'=>$page, 'type'=>$type]) }}';
    @else 
    var title_check_url = '{{ route('websites.pages.title_check', $website) }}?';
    var page_url = '{{ route('websites.pages.store', ['website'=>$website, 'type'=>$type]) }}';
    var page_url2 = '{{ route('websites.pages.index', ['website'=>$website]) }}';
    @endif
    var preview_url = '{{ route('website.preview', $website) }}';

    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-block">
                <form action="{{ route('websites.appearance', $website) }}" method="POST">
                    {{ csrf_field() }}
                    {{ method_field('PUT') }}
                    <div class="row">
                        <div class="col-lg-8">
                            ...
                            <div class="row form-group {{ $errors->has('template_id') ? ' has-error' : '' }}">
                                <label class="col-md-2 form-control-label" style="text-align:right">Template</label>
                                <div class="col-md-8">
                                    <select class="form-control" name="template_id">
                                        @foreach($templates as $template)
                                        <option value="{{ $template->id }}" {{ 
                                        (old('template_id', !empty($website->template_id) ? $website->template_id : NULL ) == $template->id) ? ' selected' : ''
                                     }}>
                                            {{ $template->name }}
                                        </option>
                                        @endforeach

<div class="col-lg-6">
    ...
    @foreach(['address', 'address_2', 'city', 'state', 'zip', 'phone'] as $field)
    ...
    <div id="socialform">
        ...
        <div class="row form-group{{ $errors->has($field) ? ' has-error' : '' }}">
            ...
            <div class="col-md-9">
                <input type="text" class="form-control" dusk="{{ $field }}" name="{{ $field }}" ref="input-{{ $field }}"
                    v-model="config.{{ $field }}.value" :placeholder="config.{{ $field }}.placeholder"
                    v-on:blur="valid_input('{{ $field }}')">
